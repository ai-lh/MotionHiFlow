...
def calc_accel(gt_joints, pred_joints):
    """
    Mean joint acceleration error
    often referred to as "Protocol #1" in many papers.
    """
    assert gt_joints.shape == pred_joints.shape, print(gt_joints.shape,
                                              pred_joints.shape)  # BxJx3
    assert gt_joints.dim() == 3
    # Expects BxJx3
    # valid_mask = torch.BoolTensor(target[:, :, 0].shape)
    accel_gt = gt_joints[:-2] - 2 * gt_joints[1:-1] + gt_joints[2:]
    accel_pred = pred_joints[:-2] - 2 * pred_joints[1:-1] + pred_joints[2:]
    normed = torch.linalg.norm(accel_pred - accel_gt, dim=-1)
    accel_seq = normed.mean(1)
    return accel_seq


def calc_pampjpe(gt_joints, pred_joints, sample_wise=True, return_transform_mat=False):
    """
    Mean joint acceleration error
    often referred to as "Protocol #1" in many papers.
    """
    # Expects BxJx3
    gt_joints, pred_joints = gt_joints.float(), pred_joints.float()
    ...
    pred_joints_tranformed, PA_transform = batch_compute_similarity_transform_torch(
        pred_joints, gt_joints)
    pa_mpjpe_each = calculate_mpjpe(gt_joints, pred_joints_tranformed)
