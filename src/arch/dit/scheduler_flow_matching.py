"""
modified from: https://github.com/jy0205/Pyramid-Flow/blob/main/diffusion_schedulers/scheduling_flow_matching.py
"""
...
class PyramidFlowMatchEulerDiscreteScheduler(SchedulerMixin, ConfigMixin):
    ...
    def init_sigmas_for_each_stage(self):
        ...
        for i_s in range(stages):
            ...
            # stage_sigmas = self._get_lognorm(1, 0, training_steps)
            # self.sigmas_per_stage[i_s] = torch.from_numpy(stage_sigmas)
            # self.timesteps_per_stage[i_s] = self.sigmas_per_stage[i_s] * (timestep_max - timestep_min) + timestep_min

    def _get_lognorm(self, start, end, num_steps):
        sigmas = np.linspace(
            1e-10, 1-1e-10, num_steps + 1,
        )
        sigmas = norm.ppf(sigmas)
        sigmas = np.exp(sigmas) / (1 + np.exp(sigmas))
        sigmas = sigmas * (end - start) + start
    ...
    def _stage_sigma_to_t(self, sigma, stage_index):
        """
        sigma: [0, 1] (1 is the start of the stage, 0 is the end of the stage)
        """
        stage_timesteps = self.timesteps_per_stage[stage_index]
        shift, scale = stage_timesteps[-1], (stage_timesteps[0] - stage_timesteps[-1])
        return shift + scale * sigma
